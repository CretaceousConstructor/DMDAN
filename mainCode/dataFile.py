...
def bciiv2a_multitask_da(aug=False, reproduce=True, dataset='B', loadN='1', subject=1, data_len='data_0-4', ea_preprocess=False):
    # data_len = 'data_1-4'
    # subject = 5
    ...
    #目标域受害者下标
    sub = subject - 1
    SourceList = list(range(allSubNum)) #从0开始到allSubNum-1
    ...
    for dataPath in SourceList:
        ...
        if(dataset == 'A'):
            raw = sio.loadmat('./data/dataset2a1000/' + str(dataPath) + '.mat')
        else:
            raw = sio.loadmat('./data/dataset2b1000/' + str(dataPath) + '.mat')

        trnData1, trnLabel1, tstData1, tstLabel1 = raw['trnData'], raw['trnLabel'][0], raw['tstData'], raw['tstLabel'][0]

        #EA处理
        if (ea_preprocess):
            trnData1 = utils.euclidean_space_data_alignment(trnData1)
            tstData1 = utils.euclidean_space_data_alignment(tstData1)
    ...
    for dataPath in targetList:
        ...
        #EA处理
        if (ea_preprocess):
            targetData = utils.euclidean_space_data_alignment(targetData)
            targetTstData = utils.euclidean_space_data_alignment(targetTstData)
